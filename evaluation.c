...
void packet_handler(const struct pcap_pkthdr *pkthdr, const unsigned char *packet) {
    ...
    if (src_ip == filter_src_ip && dst_ip == filter_dst_ip) {
...
int main(int argc, char *argv[]) {
    ...
    char *dev = "wlan1"; // Replace with your actual network device
    char filename[100];

    if (argc != 3) {
        fprintf(stderr, "Usage: %s <SrcIP> <DstIP>\n", argv[0]);
        return 1;
    }

    filter_src_ip = inet_addr(argv[1]);
    filter_dst_ip = inet_addr(argv[2]);
    ...
    // Construct the filename using source and destination IPs
    sprintf(filename, "tracking/%s_%s.pcap", argv[1], argv[2]);
