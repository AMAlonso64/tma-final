...
def block_ip():
    ...
    command = f'iptables -A FORWARD -d {ip_to_block} -j DROP'
    print(command)
    try:
...
def unblock_ip():
    ...
    command = f'iptables -D FORWARD -d {ip_to_block} -j DROP'
    print(command)
    try:
        result = subprocess.run(command, shell=True, check=True)
        #if "Aceptar" in result.stdout:
...
def connection_analysis():
    ...
    our_network_ips = extract_unique_ips(miner_connections + warning_connections, '10.42.0.0/24')
